...
const getProcessingAgreementFromSessionId = (sessionId) => `
PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
PREFIX dct: <http://purl.org/dc/terms/>
...
async function hasProcessingAgreement(req) {
  try {
    ...
    // Check cache first
    const cached = processingAgreementCache.get(sessionId);
    if (cached) {
      const now = Date.now();
      if (now - cached.timestamp < CACHE_TTL_MS) {
        console.log(`Processing agreement cache hit for session ${sessionId}: ${cached.hasAgreement}`);
        return cached.result;
      } else {
        // Cache entry expired, remove it
        console.log(`Processing agreement cache expired for session ${sessionId}`);
        processingAgreementCache.delete(sessionId);
      }
    }

    // Cache miss or expired, query the database
    console.log(`Processing agreement cache miss for session ${sessionId}, querying database`);
    ...
    // Store in cache with timestamp
    processingAgreementCache.set(sessionId, {
      hasAgreement,
      result: authResult,
      timestamp: Date.now(),
    });
    console.log(`Processing agreement cached for session ${sessionId}: ${hasAgreement}`);
