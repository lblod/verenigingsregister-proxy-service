...
export async function isAuthorized(req, { vCode = null } = {}) {
  const roleCheck = checkRole(req);
  if (!roleCheck.authorized) {
    return roleCheck;
  }

  // Check werkingsgebied for association access (when vCode is provided)
  if (ENABLE_TERRITORY_CHECK && vCode) {
    const werkingsgebiedCheck = await checkWerkingsgebied(req, vCode);
    if (!werkingsgebiedCheck.authorized) {
      return werkingsgebiedCheck;
    }
  }
...
function getWerkingsgebiedCheckQuery(sessionId, vCode) {
  return `
...
ASK {
  GRAPH ${sparqlEscapeUri(ASSOCIATIONS_GRAPH)} {
    ?association adms:identifier/generiek:gestructureerdeIdentificator/generiek:lokaleIdentificator ${sparqlEscapeString(vCode)} ;
      org:hasPrimarySite/organisatie:bestaatUit/locn:postCode ?postalCode .
  }

  GRAPH ${sparqlEscapeUri(SESSION_GRAPH)} {
    ${sparqlEscapeUri(sessionId)} ext:sessionGroup ?adminUnit .
  }

  GRAPH ${sparqlEscapeUri(ORGANISATION_GRAPH)} {
    ?adminUnit besluit:werkingsgebied ?werkingsgebied .
    ?postInfo a adres:Postinfo ;
      adres:postcode ?postalCode .
    # A Postinfo is sfWithin the werkingsgebied of its gemeente and, for
    # stad Antwerpen postcodes, of its district. Match the unit's own
    # werkingsgebied (gemeente/district login) or an ancestor of it
    # (provincie login).
    {
    ...
    UNION
    {
      ?postInfo geo:sfWithin ?area .
      ?area geo:sfWithin ?werkingsgebied .
    }
  }
}`;
}
...
/**
 * Check if the user's admin unit covers the werkingsgebied of the association's primary site.
 * Returns { authorized, detail }.
 */
async function checkWerkingsgebied(req, vCode) {
  const sessionId = req.headers['mu-session-id'];
  if (!sessionId) {
    return {
      authorized: false,
  ...
  try {
    const result = await query(getWerkingsgebiedCheckQuery(sessionId, vCode));

    console.log('Werkingsgebied check result:', JSON.stringify(result, null, 2));

    const isAuthorized = result?.boolean === true;

    return {
      authorized: isAuthorized,
      detail: isAuthorized
        ? 'Admin unit covers association werkingsgebied'
        : 'Admin unit does not cover association werkingsgebied',
    };
  } catch (error) {
    return {
      authorized: false,
      detail: `Werkingsgebied check failed: ${error.message}`,
    };
  }
}
