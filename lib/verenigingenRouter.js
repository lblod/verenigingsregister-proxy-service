...
async function buildHeaders(extra = {}) {
  ...
  // Remove all headers listed in MU_REQUEST_HEADERS from req.headers
...
async function forward(req, res) {
  ...
  const path = req.originalUrl.replace(/^\/verenigingen/, '');
  const url = `${API_BASE}${path}`;
  const headers = await buildHeaders({ 'If-Match': req.header('If-Match') });
...
router.get('/:vCode/historiek', (req, res, next) => forward(req, res).catch(next));
...
// ----- Lidmaatschap -----
router.post('/:vCode/lidmaatschappen', (req, res, next) => forward(req, res).catch(next));
router.patch('/:vCode/lidmaatschappen/:id', (req, res, next) => forward(req, res).catch(next));
router.delete('/:vCode/lidmaatschappen/:id', (req, res, next) => forward(req, res).catch(next));
