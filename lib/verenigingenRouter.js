...
import { uuid } from 'mu';
import { getSessionOVOCode, isAuthorized, validateRequestReason, logDataAccess, getSessionInfo, checkRole } from './authorizationCheck.js';
...
router.get('/:vCode/basisinformatie', async (req, res, next) => {
  try {
    const roleCheck = checkRole(req, true);
