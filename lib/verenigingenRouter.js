...
import { uuid } from 'mu';
import { isAuthorized, validateRequestReason, logDataAccess, getSessionInfo, checkRole } from './authorizationCheck.js';
...
import { API_BASE, API_VERSION, MU_REQUEST_HEADERS, FALLBACK_CLIENT_ID, ENABLE_REQUEST_REASON_CHECK } from '../constants.js';
...
// Helper: remove sensitive fields from detail API response
const SENSITIVE_FIELDS = ['vertegenwoordigers'];

function removeSensitiveFields(data) {
  if (data?.vereniging) {
    for (const field of SENSITIVE_FIELDS) {
      delete data.vereniging[field];
    }
  }
  return data;
}
...
router.head('/:vCode', async (req, res, next) => {
  try {
    ...
    const path = req.originalUrl;
    const url = `${API_BASE}${path}`;
    const headers = await buildHeaders(req, { fallbackClientId: FALLBACK_CLIENT_ID || null });
...
router.get('/:vCode/authorization-check', async (req, res, next) => {
  try {
    const { vCode } = req.params;
    const authResult = await isAuthorized(req, { vCode });
    if (!authResult.authorized) {
      console.log('Authorization pre-check failed:', authResult.detail);
...
// ----- Basisinformatie (detail API, sensitive data stripped) -----
router.get('/:vCode/basisinformatie', async (req, res, next) => {
  try {
    const roleCheck = checkRole(req);
    if (!roleCheck.authorized) {
      console.log('Authorization failed for basisinformatie:', roleCheck.detail);
      return res.status(401).json({
        error: 'Unauthorized',
    ...
    const { vCode } = req.params;
    const url = `${API_BASE}verenigingen/${vCode}`;
    const headers = await buildHeaders(req, { fallbackClientId: FALLBACK_CLIENT_ID || null });

    const response = await axios({ url, method: 'GET', headers });

    ['etag', 'vr-sequence'].forEach((h) => {
      if (response.headers[h]) res.setHeader(h, response.headers[h]);
    });
    ...
    return res.status(response.status).json(removeSensitiveFields(response.data));
  } catch (error) {
    next(error);
  }
});
...
router.get('/:vCode', async (req, res, next) => {
  ...
  try {
    ...
    // Check authorization (includes role and werkingsgebied checks)
    const authResult = await isAuthorized(req, { vCode });
    if (!authResult.authorized) {
      await logDataAccess({
        resourceUri,
